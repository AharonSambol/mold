...
use crate::mold_ast::{FuncTypes, PPT, VarTypes};
use crate::types::{CHAR_TYPE, FLOAT_TYPE, generify, INT_TYPE, STR_TYPE, Type, TypeKind, TypName, unwrap, unwrap_u};
...
pub fn add_types(
    ...
    vars: &mut VarTypes, funcs: &FuncTypes,
    ppt: &PPT
) {
    ...
    match &ast[pos].value {
        AstNode::ForStatement => {
            ...
            //3 ****************************************************************
            add_types(ast, unwrap_u(&ast[children[0]].children)[1], vars, funcs, ppt);
            ...
            add_types(ast, children[1], vars, funcs, ppt);
            vars.pop();
        }
        ...
        => {
            ...
            for child in children {
                add_types(ast, child, vars, funcs, ppt);
            }
            vars.pop();
        }
        ...
        AstNode::Property => {
            add_types(ast, children[0], vars, funcs, ppt);
            ...
            ast[pos].typ = Some(Type {
                ...
                children: Some(vec![
                    ...
                    generify(&children.iter().map(|x| {
                        add_types(ast, *x, vars, funcs, ppt);

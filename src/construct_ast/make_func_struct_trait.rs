...
use crate::construct_ast::find_generics::{find_generics_in_typ, get_generics, is_generic};
...
pub fn make_func(
    ...
    add_to_tree(index, ast, Ast::new(AstNode::ArgsDef));
    add_to_tree(index, ast, Ast::new(AstNode::ReturnType));
    add_to_tree(index, ast, Ast::new(AstNode::Body));
...
pub fn make_struct(
    ...
    add_to_tree(parent, ast, Ast::new(AstNode::Struct(name.clone())));
    let index = ast.len() - 1;
    ...
    *structs.get_mut(&name).unwrap() = StructType { generics: Some(generics_names), pos: index};
    ...
    add_to_tree(index, ast, Ast::new(AstNode::ArgsDef));
    add_to_tree(index, ast, Ast::new(AstNode::Module));
    add_to_tree(index, ast, Ast::new(AstNode::Traits));
    let args_pos = ast.len() - 3;
...
pub fn make_trait(
    ...
    add_to_tree(parent, ast, Ast::new(AstNode::Trait(name.clone())));
    let index = ast.len() - 1;
    ...
    *traits.get_mut(&name).unwrap() = TraitType { generics: Some(generics_names), pos: index};
    ...
    while let SolidToken::Def = &tokens[pos] {
        ...
        add_to_tree(body_pos, ast, Ast::new(AstNode::Function(func_name.clone())));
        ...
        //1 args
        add_to_tree(func_pos, ast, Ast::new(AstNode::ArgsDef));
